...
// Avatar coverage (set/replace/clear, and the 400/413/415 edge cases) moved
// to `tests/avatar_api.rs` along with the rest of `PATCH /actors/me`'s
// avatar handling — see that file's module doc for why: it needs to send
// real multipart bodies through the router, which this file's `Storage`
// (bound to an unreachable address) can't support.
